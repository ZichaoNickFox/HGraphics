      ...
      , viewportState      = Just . SomeStruct $ zero
        { viewports =
          [ Viewport
              ...
              , height   = realToFrac swapchainExtent.height
              , minDepth = 0
              , maxDepth = 1
              }
          ]
...
windowInstanceCreateInfo window = do
  ...
  let requiredLayers     = []
      ...
      requiredExtensions = [KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME, EXT_DEBUG_UTILS_EXTENSION_NAME] <> windowExtensions
...
pickGraphicalPhysicalDevice inst surface _requiredExtensions desiredFormat = do
 ...
 where
  ...
  deviceScore dev = do
    ...
    let totalSize = sum $ (\heap -> heap.size) <$> heaps
    pure totalSize
  ...
  getFormat dev = do
    ...
    pure $ case formats of
      ...
      _
        | V.any
          (\f ->
            f.format == desiredFormat.format && f.colorSpace == desiredFormat.colorSpace
          )
          formats
        -> desiredFormat
      _ -> V.head formats
